...
import { adMap } from './utils/ad-mapping'
import AdUnit from './children/AdUnit'
import ArcAdminAd from './children/ArcAdminAd'
...
export const R7ArcAdDisplay = props => {
  ...
  const { blockLayout = 'background' } = customFields
  ...
  return (
    <Stack id={`arcad-feature-${instanceId}`} className={BLOCK_CLASS_NAME} alignment="center">
      ...
      {isAdmin ? <ArcAdminAd {...config} layout={blockLayout} /> : null}
    </Stack>
  )
}
...
const AdsR7 = props => {
  ...
  console.log('ww', propsWithContext)
