...
const environment = process.argv.indexOf('--production') > -1 ? 'PROD' : 'SANDBOX'
...
const bundleName = `${new Date().toISOString()}-${commit_hash}${branch_name}`.replaceAll(
  /[^a-zA-Z0-9-]/gi,
  '-',
)
...
const deploy = async () => {
  try {
    await axios.post(`${baseURL}services?bundle=${bundleName}&version=${pbVersion}`, null, {
      headers: {
        Authorization: `Bearer ${authToken}`,
      },
    })
...
const getLatestServiceVersion = response => {
  ...
  console.log(`latestServiceVersion: ${latestServiceVersion}`, `currentVersion: ${currentVersion}`)
...
const promote = async version => {
  ...
  try {
    if (!version) {
      throw new Error('The version number argument passed to the promote function is falsy')
    }
...
const terminateOldest = async () => {
  ...
  if (nonLiveServices.length) {
    const oldest = nonLiveServices.reduce((currentOldest, currentService) => {
      if (currentOldest.LastModified.localeCompare(currentService.LastModified) > 0) {
...
const deployDev = async () => {
  ...
  await notification('Successful deployment!')
}
